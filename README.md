...
<head>
  ...
  <title>Ngoulla Morel — Portfolio</title>

  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;800&display=swap" rel="stylesheet">

  <style>
    :root{
      --bg1:#0f172a; --bg2:#0f2540; --accent:#7c3aed;
    ...
    *{box-sizing:border-box}
    ...
    .wrap{position:relative; z-index:1; max-width:1000px; margin:0 auto; padding:56px 24px}
    header{display:flex; align-items:center; justify-content:space-between; gap:16px}
    .brand{font-weight:800; letter-spacing:-0.5px}
    nav a{color:inherit; text-decoration:none; margin-left:18px; opacity:0.9}
    nav a:hover{opacity:1; text-decoration:underline}

    .hero{display:flex; align-items:center; gap:30px; margin-top:40px}
    .hero-left{flex:1}
    .title{font-size:clamp(28px,5vw,44px); margin:0 0 8px}
    .subtitle{margin:0 0 18px; color:#bcd2ff; font-weight:300}
    .typewriter{color:var(--accent); font-weight:600}

    .cta{display:inline-block; padding:10px 16px; background:linear-gradient(90deg,var(--accent),#5eead4);
         color:#041024; border-radius:10px; text-decoration:none; font-weight:700; transition:transform .2s ease}
    .cta:hover{transform:scale(1.05)}

    .card{width:220px; padding:18px; background:var(--glass); border-radius:14px; backdrop-filter:blur(6px);
         box-shadow:0 6px 22px rgba(2,6,23,0.6); text-align:center}
    .avatar{width:84px; height:84px; border-radius:50%; object-fit:cover; border:3px solid rgba(255,255,255,0.06)}
    .small{font-size:13px; color:#cbd9ff}
    .btn-group{margin-top:12px; display:flex; flex-direction:column; gap:8px}
    .btn{padding:8px; background:var(--accent); border-radius:8px; text-decoration:none; color:white; font-size:14px; font-weight:600; transition:background .2s ease}
...
<body>
  ...
  <div class="wrap">
    ...
    <main>
      <div class="hero">
        ...
        <aside class="card reveal">
          ...
          <div class="btn-group">
            <a class="btn" href="https://whatsapp.com/channel/0029Vb5p1DHI7Be7UmI7BW0f" target="_blank">WhatsApp Channel</a>
    ...
    <footer class="reveal">Made with ♥ • © <span id="year"></span> Ngoulla Morel</footer>
  </div>
